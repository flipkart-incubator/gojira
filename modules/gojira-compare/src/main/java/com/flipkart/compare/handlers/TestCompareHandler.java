...
public abstract class TestCompareHandler {
    ...
    try {
      ...
      if (diffDetails != null && !diffDetails.isEmpty()) {
        for (DiffDetail diffDetail : diffDetails) {
          ...
          logger.error(
              "not ignored DiffDetail | " + diffDetail.getDiffType() + ":" + diffPathForIgnoreRule +
                  "suggested ignore pattern | " + diffPathForIgnoreRule
  ...
  public final boolean canBeIgnored(DiffDetail diffDetail) {
    ...
    if (diffDetail.getDiffType().equals(DiffType.ADD) && diffIgnoreRepository
        .getDiffIgnorePatterns().containsKey(DiffType.ADD)) {
    ...
    if (diffDetail.getDiffType().equals(DiffType.MOVE) && diffIgnoreRepository
        .getDiffIgnorePatterns().containsKey(DiffType.MOVE)) {
    ...
    if (diffDetail.getDiffType().equals(DiffType.REMOVE) && diffIgnoreRepository
        .getDiffIgnorePatterns().containsKey(DiffType.REMOVE)) {
    ...
    if (diffDetail.getDiffType().equals(DiffType.MODIFY) && diffIgnoreRepository
        .getDiffIgnorePatterns().containsKey(DiffType.MODIFY)) {
   ...
   *                 TestCompareHandler#canBeIgnored(DiffDetail)} to check if the {@link DiffDetail}
   *                 can be ignored. and returns
   * @return null or {@link DiffDetail} object accordingly
   */
  public final DiffDetail getDiffDetail(String diffKey, Object expected, Object actual,
      DiffType diffType) {
    DiffDetail.Builder diffDetailBuilder = DiffDetail.builder()
        .setDiffPath(diffKey)
