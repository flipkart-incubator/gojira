...
/**
 * Guice module that needs to be installed for enabling comparison. It handles which compare
 * handlers to pick from CompareHandlerRepository and allows for setting up of ignorable {@link
 * DiffDetail}
 */
public class ComparisonModule extends AbstractModule {
  ...
  protected void configure() {
    CompareHandlerRepository compareHandlerRepository = new CompareHandlerRepositoryImpl();
    compareHandlerRepository.setDefaultCompareHandler(comparisonConfig.getDefaultCompareHandler());
    bind(CompareHandlerRepository.class).toInstance(compareHandlerRepository);

    DiffIgnoreRepository diffIgnoreRepository = new DiffIgnoreRepositoryImpl();
    diffIgnoreRepository.setupDiffIgnorePatterns(comparisonConfig.getDiffIgnoreMap());
