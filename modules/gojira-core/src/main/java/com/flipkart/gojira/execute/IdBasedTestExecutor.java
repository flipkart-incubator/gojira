...
public class IdBasedTestExecutor<T extends TestDataType> {
  ...
  private TestSerdeHandler testDataSerdeHandler = GuiceInjector.getInjector()
      .getInstance(SerdeHandlerRepository.class).getTestDataSerdeHandler();
  // Make this per request type.

  public void execute(String testId)
      throws TestExecutionException, SinkException, TestSerdeException {
    TestData<TestRequestData<T>, TestResponseData<T>, T> testData = testDataSerdeHandler
        .deserialize(sinkHandler.read(testId), TestData.class);
   ...
   * @throws SinkException          exception thrown if we are not able to read data from {@link
   *                                SinkHandler} implementation
